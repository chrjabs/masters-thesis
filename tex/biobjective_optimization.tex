...
Multi-objective optimization is the problem of optimizing $\nobj$ objective functions $\generalobj_i: \feasible \rightarrow \mathbb{R}^+$ with the decision variable $x$, while $x$ is from a feasible set $\feasible$~\autocite{Ehrgott2005-1}.
...
Both lexicographic optimization and lexicographic max-ordering optimization are general for any number of objectives, however, here we discribe them in the context of bi-objective optimization.
...
\section{Related Approaches to Bi-Objective Optimization\label{sec:approaches}}
