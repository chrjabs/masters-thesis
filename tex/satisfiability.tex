...
% SAT solvers
A SAT solver~\autocite{handbook2-cdcl} is an implementation of an algorithm that determines the satisfiability of a given formula $\formula$.
...
\TODO{works on partial assignments -> decisions and unit propagation -> conflict analysis (never see partial assignment again) -> non-chronological backtracking (remove some number of most-recent of recent decisions) -> search terminates at solution or when learning empty clause (empty clause entailed by formula)}
...
\TODO{sat has shown to be impactful, solving instances with up to millions of problems}
...
MaxSAT algorithms using SAT solvers can be further characterized w.r.t.\ how the objective function is modelled:
