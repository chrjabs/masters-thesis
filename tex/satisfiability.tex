...
Next, we discuss incremental SAT solving and cardinality constraints.
Both of these topics are prerequisites for maximum satisfiability, which is discussed in \cref{sec:max-sat}.
...
This clause contains information learned from the found conflict and retains it for the remainder of the search process.
...
\begin{example}\label{ex:sat-modelling}
  ...
  The groups of ingredients the guests like form three \emph{sets}, and you want to know if a \emph{cover} (another collection of ingredients) of at most size two exists, so that this cover intersects with all three of the sets.
...
For applications where only either upper or lower bounding on the number of literals assigned to true is needed, the size of the encoded totalizer can be reduced by encoding implications in only one direction instead of in both (i.e., $\ov{L}{b} \leftarrow (\sum_{l \in L} l < b)$ or $\ov{L}{b} \rightarrow \sum_{l \in L} l < b$, but not both, depending on if upper or lower bounding is needed).
...
Each clause $\clause \in \softs$ is assigned a weight $w_\clause$ and a solution $\sol$ that satisfies $\formula$ while achieving minimum weight of falsified soft clauses $\sum_{\clause\in\softs \mid \tau(\clause)=0} w_\clause$ is optimal for the problem.
...
\begin{example}\label{ex:maxsat-modelling}
  ...
  After we modify the instance to account for guest C liking peas, the MaxSAT solver might give us the optimal solution $\sol$ with $\sol(v_\text{bell pepper}) = \sol(v_\text{peas}) = 1$ and $\sol(v) = 0$ for all other $v$.
