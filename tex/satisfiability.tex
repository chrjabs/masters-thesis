...
The advantage of using SAT as a declarative programming language for solving other problems comes from the fact that---even though SAT is $\mathcal{NP}$-complete, and it is unclear if a polynomial time algorithm for solving it exists---conflict-driven clause learning solvers for SAT are efficient in practice and can solve instances with millions of variables and clauses~\autocite{handbook2-cdcl}.
