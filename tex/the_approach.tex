...
Solution-improving search works by---starting from the known upper bound $b=\Obj_\dec(\sol)$---iteratively invoking the SAT solver under the assumptions $\{\ov{\Obj_\dec}{b}, \ove{\Obj_\inc}{b_\inc}\}$ for decreasing values of $b$ until the solver returns \unsat{}.
...
\begin{example}\label{ex:main-iteration}
  ...
  As the result is satisfiable and the solver returns, e.g., the solution $\sol^c_4 = \solcfour$,
...
The procedure terminates when the result is unsatisfiable, after which (on \cref{ln:su-ret}) the value of $b$ and the solution obtained during the final satisfiable call are returned as $b_\inc$ and $\sol$.  
...
\begin{example}\label{ex:satunsat}
  ...
  The result is unsatisfiable, so the procedure returns $b_\inc = 2$ and $\sol^o_2$.
  \looseness=-1
...
\begin{example}
  ...
  The result is unsatisfiable, so the totalizer is extended to $\tot(\Obj_\inc, 1)$ and the SAT solver invoked with the assumptions $\{ \ove{\Obj_\inc}{1}\}$.
  The result is satisfiable.
