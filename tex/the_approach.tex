...
In the beginning of each main loop iteration, the procedure $\Min$ is employed to minimize the increasing objective, i.e., compute the smallest value $b_\inc$ for which there is a solution $\sol$ with $\Obj_\inc(\sol) = b_\inc$ and $\Obj_\dec(\sol) < b_\dec$ (\cref{l:minim1}). 
...
Next, the algorithm employs \emph{solution-improving search}~\autocites{handbook2-maxsat,DBLP:journals/jsat/BerreP10,DBLP:journals/jsat/EenS06} to minimize the decreasing objective, i.e., to compute the smallest $b_\dec$ for which there is a solution $\sol$ with $\Obj_\inc(\sol) = b_\inc$ and $\Obj_\dec(\sol) = b_\dec$ (\cref{l:minim2}).
...
Solution-improving search works by---starting from the known upper bound $b=\Obj_\dec(\sol^r)$---iteratively invoking the SAT solver under the assumptions $\{\ov{\Obj_\dec}{b}, \ove{\Obj_\inc}{b_\inc}\}$ for decreasing values of $b$ until the solver reports \unsat{}.
As soon as unsatisfiability is reached, $\Simpr$ returns $b_\dec=b+1$ and the last satisfying solution $\sol^r$ for which $\Obj_\inc(\sol^r) = b_\inc$ and $\Obj_\dec(\sol^r) = b_\dec$.
...
\begin{algorithm}[t]
  ...
  \textbf{Output}: Solution $\sol^r$ and smallest $b=\Obj_\inc(\sol^r)$ so that $\Obj_\dec(\sol^r)<b_\dec$.

  \begin{algorithmic}[1]
    \STATE build or extend $\tot(\Obj_\inc,b)$ if necessary \label{ln:su-tot}
    \STATE $(\res,\sol^r) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ov{\Obj_\inc}{b}\})$
    \WHILE{$\res = \sat$}
      \STATE $b \gets \Obj_\inc(\sol^r)$
      \STATE $(\res,\sol^r) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ov{\Obj_\inc}{b}\})$ \label{ln:su-query}
    \ENDWHILE
    \STATE \textbf{return} $(b,\sol^r)$ \label{ln:su-ret}
  \end{algorithmic}
\end{algorithm}
...
As inputs, the procedure gets the last bound $b_\dec$ on $\Obj_\dec$ and the upper bound $b=\Obj_\inc(\sol^r)$ on the minimum of the increasing objective known from the last SAT solver call.
...
The procedure terminates when the result is \unsat{}, after which on \cref{ln:su-ret}, the value of $b$ and the solution obtained during the final satisfiable call are returned as $b_\inc$ and $\sol^r$.  
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $(\res,\sol^r) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ove{\Obj_\inc}{b+1}\})$
    \WHILE{$\res = \unsat$}
      \STATE $b \gets b+1$
      \STATE extend $\tot(\Obj_\inc,b+1)$ \label{ln:us-tot}
      \STATE $(\res,\sol^r) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ove{\Obj_\inc}{b+1}\})$ \label{ln:us-query}
    \ENDWHILE
    \STATE \textbf{return} $(b+1,\sol^r)$ \label{ln:us-ret}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $(\res,\sol^r,\core) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ove{\Obj_\inc}{b}\} \cup \{\lnot l \mid l \in \Obj_\inc \setminus \Act\})$ \label{ln:msu-firstquery}
    \WHILE{$\res = \unsat$}
      ...
      \STATE $(\res,\sol^r,\core) \gets \satsolver(\{\ov{\Obj_\dec}{b_\dec}, \ove{\Obj_\inc}{b}\} \cup \{\lnot l \mid l \in \Obj_\inc \setminus \Act\})$ \label{ln:msu-mainquery}
    \ENDWHILE
    \STATE \textbf{return} $(b,\sol^r)$
  \end{algorithmic}
\end{algorithm}
...
The procedure continues until the SAT solver returns satisfiable, and a solution $\sol^r$ which has $\Obj_\inc(\sol^r) = b$ and $\Obj_\dec(\sol^r) < b_\dec$ is found.
...
For enforcing $\Obj_\inc(\sol)\le b_\inc$ when employing \msu{}, consider an invocation of $\msu(b_\dec, b_\inc)$ made during \algname{} and assume it returns the tuple $(b_\inc, \sol^r)$. 
...
\TODO{Do I need to extend this? If yes, with what?}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \IF{$|\Act| < \thr \cdot |\Obj_\inc|$}
      \STATE $(b_\inc,\sol^r) \gets \msu{}(b_\dec, b_\inc)$ \COMMENT{Immediately terminates once $|\Act| < \thr \cdot |\Obj_\inc|$ is met}\label{ln:msh-msu}
    \ENDIF
    \IF{$|\Act| \ge \thr \cdot |\Obj_\inc|$}
      \STATE fully build or extend $\tot(\Obj_\inc, b)$ if necessary \label{ln:msh-tot}
      \STATE $(b_\inc,\sol^r) \gets \satunsat{}(b_\dec, b)$ \label{ln:msh-su}
    \ENDIF
...
The subroutine---that takes the last bound $b_\dec$ on $\Obj_\dec$, the known upper $b=\Obj_\inc(\sol^r)$ and lower bounds $b_\inc$ on the minimum of $\Obj_\inc$ as parameters---is outlined in \cref{alg:msh}.
...
An improved, but still generic way of blocking a solution is to build a blocking clause over all variables that the SAT solver made decisions for. \TODO{is this correct and enough?}
