...
% What is MLIC
Recently, a variety of SAT and MaxSAT-based approaches have been developed for learning interpretable classifiers from data~\autocites{DBLP:conf/ijcai/Ignatiev0NS21,DBLP:conf/cp/MaliotovM18,DBLP:conf/ijcai/NarodytskaIPM18,DBLP:conf/ijcai/Hu0HH20,DBLP:conf/cp/YuISB20,DBLP:conf/cade/IgnatievPNM18}.
...
In preliminary experiments, we found that choosing the objectives as described here leads to better performance than swapping them.
...
\TODO{Seesaw-SAT and Seesaw-SAT-min will be included once implemented and run!}
...
For the decision rule instances however, the objectives are structurally different, so the results will look different when swapping the objectives.
\TODO{Can or should I say this if we don't have experiments for it?}
...
\Cref{tab:nsolved} also summarizes the number of solved instances for all approaches applicable to this task.
