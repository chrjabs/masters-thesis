...
\usepackage{csquotes}
\usepackage[hyphens]{url} % For nicer URLs (here because of options clash)
...
\usepackage[style=numeric-comp,bibstyle=trad-plain,backend=biber,natbib=true,maxbibnames=150,giveninits=true,uniquename=init,safeinputenc,sorting=none]{biblatex}
...
\usepackage[final]{microtype} % For better typesetting
...
\usepackage{framed}
\renewenvironment{leftline}{%
  \def\FrameCommand{{\vrule width 0.75pt} \hspace{8pt}}%
  \MakeFramed {\advance\hsize-\width \FrameRestore}}%
 {\endMakeFramed}
\declaretheoremstyle[
  preheadhook=\begin{leftline},%
  postfoothook=\end{leftline},%
]{customTheorem}
\declaretheorem[style=customTheorem,numberwithin=chapter]{theorem}
\declaretheorem[style=customTheorem,numberlike=theorem]{proposition}
\declaretheorem[style=customTheorem,numberlike=theorem]{definition}
\declaretheorem[style=customTheorem,numberlike=theorem]{observation}
\declaretheorem[style=customTheorem,numberlike=theorem]{example}
...
\title{Extending Maximum Satisfiability To Bi-objective Boolean Optimization}
...
\keywords{Bi-objective optimization, MaxSAT, incremental SAT, interpretable classifiers, bi-objective set covering}
...
\include{tex/satisfiability}
\include{tex/biobjective_optimization}
\include{tex/the_approach}
