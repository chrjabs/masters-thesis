...
For a set $L$ of literals and a bound $k \in \mathbb{N}$, $\texttt{As-CNF}\left(\sum_{l \in L} l \geq k\right)$ denotes a CNF formula that encodes the linear inequality $\sum_{l \in L} l \geq k$. %, where $\circ \in \{ ,< ,> ,\geq, \leq, =\}$. %Numerous methods of forming such CNF formulas are known~\cite{DBLP:conf/cp/BailleuxB03}.
