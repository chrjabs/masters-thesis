...
\begin{frame}{Motivation --- Multiple Objectives}
  ...
  \begin{columns}
    \begin{column}{.48\textwidth}
      \emph{Task}: Choose a flat with \alert<2|handout:0>{at least two rooms} that is as cheap as possible while also having \alert<1>{as short of a commute distance as possible}.
...
\begin{frame}{Motivation --- Implicit Solution Set}
  ...
  \begin{columns}
    \begin{column}{.48\textwidth}
      \emph{Task}: From all valid decision trees, find the ones optimal with respect to size and classification error
...
\begin{frame}{Search Progression of \algname{}}
  ...
  \begin{columns}
    ...
    \begin{column}{.48\textwidth}
      \onslide<9->{
        \begin{itemize}
          \item Enumeration from top left to bottom right
        \end{itemize}
      }
    \end{column}
  \end{columns}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{groupplot}[
        ...
        \only<3->{
          \addplot [semithick,color5,dashed,mark=x] table [x=nsolved,y=pminimal,col sep=comma] {../plots/mlic-cactus.csv};
        }
        \addlegendimage{color5,dashed,semithick,mark=x}
        \addlegendentry{Competitor 1}

        \only<2->{
          \addplot [semithick,color6,dashdotted,mark=o] table [x=nsolved,y=seesaw,col sep=comma] {../plots/mlic-cactus.csv};
        }
        \addlegendimage{color6,dashdotted,semithick,mark=o}
        \addlegendentry{Commpetitor 2}
...
\begin{frame}{Results --- Instance Runtimes}
  ...
  \begin{columns}
    \begin{column}{.48\textwidth}
      \begin{centering}
        \begin{tikzpicture}
          ...
          \only<3->{
            \addplot [only marks,color0,mark=x] table [x=pminimal,y=bioptsat-msh,col sep=comma] {../plots/setcover-element-prob-msh-scatter-single.csv};
          }
          \addlegendimage{only marks,color0,mark=x}
          \addlegendentry{SetCovering 1}

          \only<4->{
            \addplot [only marks,color1,mark=o] table [x=pminimal,y=bioptsat-msh,col sep=comma] {../plots/setcover-set-card-msh-scatter-single.csv};
          }
          \addlegendimage{only marks,color1,mark=o}
          \addlegendentry{SetCovering 2}

          \only<2->{
            \addplot [only marks,color2,mark=+] table [x=pminimal,y=bioptsat-msh,col sep=comma] {../plots/mlic-msh-scatter-single.csv};
          }
    ...
    \begin{column}{.48\textwidth}
      \onslide<5->{
        \begin{itemize}
          \item Outperformed both competitors
...
\begin{frame}{Pareto Optimality}
  \begin{definition}[Domination]
    Given two objective functions $\generalobj_1,\generalobj_2$ and two solutions $x,x'$, $x$ dominates $x'$ if (i)~$\generalobj_i(x) \leq \generalobj_i(x')$ for all $i\in\{1,2\}$, and (ii)~$\generalobj_i(x) < \generalobj_i(x')$ for some $i\in\{1,2\}$.
    We represent $x$ dominating $x'$ as $x \prec x'$.
  \end{definition}

  \begin{definition}[Pareto optimality]
    A solution $x$ is Pareto-optimal iff there is no $x'$ such that $x' \prec x$, i.e., $x$ is not dominated by any other solution.
  \end{definition}
\end{frame}
